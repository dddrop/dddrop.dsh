...
- Adds a `Pavo` Settings page for repository configuration, global Archive visibility, and a read-only view of DSH Workspaces.

Pavo always defines `backlog`, `ready`, `in-progress`, `done`, and `archive`. Their IDs, display names, order, required status, and transitions are fixed in source and cannot be configured. Archive remains available as a Work destination and may be shown or hidden globally on the Board and Flow Canvas from Pavo Settings. Active Works cannot be archived, and a Work cannot be archived while another Work depends on it.
...
Profile-level `columns` and `columnSettings` configuration is rejected. Settings → Pavo exposes repository settings and the Archive visibility switch; all five Columns are fixed in source.
